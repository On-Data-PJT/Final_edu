...
> 강의 표준화를 위한 강의 운영 현황 비교 분석 솔루션

`Study Labs`는 기존 LMS로는 파악하기 어려웠던 교실 안의 비정형 교육 데이터를 AI를 활용해 분석 가능한 정형 데이터로 변환 및 시각화하여 제공하는 강의 표준화 솔루션입니다.  

사용자가 업로드한 표준 커리큘럼(강의계획서), 강의자료, 강의 영상, 수강생 강의평가서(VOC)를 기반으로 실제 수업 운영 현황을 비교·분석하여 편차를 진단하며, 최근 관련 업계 동향 분석을 결합한 다각도 인사이트를 제공합니다.



## 1. 왜 필요한가

본 솔루션의 메인 타겟 사용자는 학원 및 기관 등의 교육 운영자이며, 여러 개의 동일 과정을 운영하고 있는 교육 기관이 여러 명의 강사 투입 시 겪는 강의 표준화에 대한 어려움을 해결하고자 했습니다. 또한, 표준 커리큘럼 및 타 강사 대비 자신의 실제 수업 구성 현황을 객관적으로 비교하고 파악하고 싶은 교강사를 서브 타겟 사용자로 고려했
습니다. 
...
| --- | --- | --- |
| 교육 운영자 | 동일 과정의 강사별 편차를 객관적으로 파악하기 어려움 | 데이터에 기반한 효율적인 선제적 리스크 관리 및 표준화 실현 |
| 교강사 | 자신의 실제 수업이 표준안이나 동료 강사와 얼마나 다른지 파악하기 어려움 | 데이터에 기반한 객관적인 자가 점검 |
| 수강생 | 동일한 과정 내에서도 수강 시기 또는 강사에 따라 차이가 발생함 | 표준화된 커리큘럼을 통한 균등한 학습 기회 |
...
2. `VOC 기반 불만 및 만족 요인 분석`
   - 수강생 평가서에서 반복 불만, 긍정 포인트, 개선 포인트를 추출합니다.
3. `Gap 기반 운영 인사이트`
...
본 솔루션의 핵심 데이터 처리 및 분석 흐름은 아래와 같습니다.
...
이 단계의 목표는 모든 입력을 하나의 분석 엔진이 다룰 수 있는 공통 포맷으로 바꾸는 것입니다.
