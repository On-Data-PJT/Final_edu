...
`Final Edu`는 동일한 과정을 여러 강사가 운영할 때 발생하는 수업 편차를 줄이기 위한 공모전용 MVP입니다.  
...
Final Edu는 교육 현장의 비정형 데이터를 분석 가능한 형태로 바꿔 세 가지 축으로 보여줍니다.
...
Final Edu는 교육 운영 현장에서 가장 설명하기 어려운 문제인 `같은 과정을 가르치지만 실제로는 다르게 전달되는 수업`을 데이터로 보여주기 위한 프로젝트입니다.  
